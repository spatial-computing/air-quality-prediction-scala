...
object IDWTesting {
  ...
  def prediction(config: Map[String, Any], sparkSession: SparkSession): Unit = {
    ...
    val schema = new StructType()
      .add(StructField("sensor_id", StringType))
      .add(StructField("timestamp", TimestampType))
      .add(StructField("aqi", DoubleType))
      .add(StructField("prediction", DoubleType))
      .add(StructField("error", DoubleType))
      .add(StructField("sqrError", DoubleType))

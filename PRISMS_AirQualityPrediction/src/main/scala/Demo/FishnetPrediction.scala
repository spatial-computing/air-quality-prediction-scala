...
object FishnetPrediction {
  ...
  def fishnetPrediction(config: Map[String, Any], sparkSession: SparkSession): Unit = {
    ...
    val fishnetGid = DBConnection.dbReadData(fishnetTableName, fishnetColumnSet, "", sparkSession)
    ...
    if (config("current") == true) {
      ...
      val maxTimestamp = DBConnection.dbReadData(airQualityTableName, List(s"max(${airQualityColumnSet(1)}) as max_timestamp"), conditions, sparkSession)
    ...
    if (config("from_time_to_time") == true) {
      ...
      for (eachTime <- times) {
        ...
        if (dt.count() >= 10) {
          ...
          val result = Prediction.predictionRandomForest(dt, trainingContext, testingContext, config)
            .select(fishnetColumnSet.head, predictionColumn)
            .withColumn("timestamp", functions.lit(eachTime))
            .select(fishnetColumnSet.head, "timestamp", predictionColumn)
